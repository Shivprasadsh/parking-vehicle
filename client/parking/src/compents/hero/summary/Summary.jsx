...
const SummaryParking = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get('https://parking-vehicle-zbdl.vercel.app/?vercelToolbarCode=daRpxUF3FYwdH-y/api/summary');
        setRecord(res.data);
      } catch (error) {
  ...
  const removeVehicle = async (vehicleNo, parkingSlot) => {
    try {
      const response = await axios.post('https://parking-vehicle-zbdl.vercel.app/?vercelToolbarCode=daRpxUF3FYwdH-y/api/removeVehicle', {
        vehicleNo,
        parkingSlot
      });
  ...
  const checkInVehicle = async (vehicleNo, parkingSlot) => {
    try {
      const response = await axios.post('https://parking-vehicle-zbdl.vercel.app/?vercelToolbarCode=daRpxUF3FYwdH-y/api/chenkinVhl', {
        vehicleNo,
        parkingSlot
      });
